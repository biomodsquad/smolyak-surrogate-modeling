...
class Surrogate:
    ...
    surrogate.
    ``norm`` is a :class:`Normalizer` object that can perform a pre-training
    transformation on the data used to fit the surrogate prior to fitting, then
    do the inverse of the transformation on the surrogate's output. 
...
class GradientSurrogate(Surrogate):
    ...
    `domain`, `grid_generator`, and `norm can be defined as in :class:`Surrogate`.
