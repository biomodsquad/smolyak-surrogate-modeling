...
class Surrogate:
    ...
    is generated (:meth:`_make_basis_matrix`) by evaluating basis
    ...
    def derivative(self, x):
        ...
        for ni in range(self.dimension):
            ...
            for coeff, basis in zip(self.coefficients,
                ...
                if self.dimension > 1:
                    term = numpy.prod(
                        [f.derivative(x) if dim == ni else f(x) for x, f, dim
                         in zip(x_scaled, basis, range(self.dimension))])
                else:
                    term = coeff*basis.derivative(x_scaled[0])
                value += coeff*term
            gradient.append(value)

        return gradient[0] if self.dimension == 1 else gradient

    def _make_basis_matrix(self, gradient=False):
        """Create basis matrix for set of points and functions.

        A basis matrix can be created using a set of N n-dimensional
        points and basis functions. The basis matrix is N x N in cases
        where creating a surrogate using a function is the goal, and
        nN x N in cases where creating a surrogate using a function's
        derivatives is the intention. The parameter `gradient=True` tells
        the function to generate the basis matrix of the basis function's
        derivatives.
        The basis functions (or gradient) are evaluated at one point at
        each row (The derivative of basis functions at each point will be
        evaluated in all dimensions to make the basis matrix where
        `gradient=True`).
        Then, using this matrix, the surrogate's coefficients can be
        determined.

        Parameters
        ----------
        gradient: bool
            True for `GradientSurrogate` and False for `Surrogate`,
        ...
        basis_functions = self.grid.basis_functions
        count = -1
        # create basis matrix with appropriate size
        basis_matrix = numpy.zeros((self.dimension * len(points)
                                    if gradient else len(points), len(points)))
        # compute number of equations per point
        eval_per_point = basis_matrix.shape[0]//basis_matrix.shape[1]
        # evalaute each term
        for i, point in enumerate(points):
            for ni in range(eval_per_point):
                count = i if not gradient else count + 1
                for j, basis in enumerate(basis_functions):
                    if gradient:
                        if self.dimension > 1:
                            value = numpy.prod([f.derivative(x) if dim == ni
                                                else f(x) for x, f, dim in zip(
                                                    point, basis, range(
                                                     self.dimension))])
                        else:
...
class GradientSurrogate(Surrogate):
    ...
    :meth:`derivative` takes the derivative of the surrogate at a given point.
    ...
    def train_from_data(self, data):
        ...
        basis_matrix = self._make_basis_matrix(gradient=True)
        self._coefficients = numpy.linalg.lstsq(basis_matrix, data,
                                                rcond=None)[0]
