...
from smolyay.grid import (IndexGridGenerator, SmolyakGridGenerator, IndexGrid, 
                          generate_compositions)
import numpy


def test_initial_nested_basis_set_points():
    ...
    assert numpy.allclose(test_class._points,
                          [0, -1.0, 1.0, -0.70710, 0.70710], atol=1e-10)


def test_initial_nested_basis_set_functions():
...
def test_initial_nested_basis_set_levels():
...
def test_grid_points_one_dimension():
...
def test_grid_points_basis_one_dimension():
...
def test_make_integer_grid_points_one_dimension():
    """Test integer grid points for one dimension."""
    test_nested_basis_set = ChebyshevFirstKind.make_nested_set(2)
    test_class = SmolyakGridGenerator(test_nested_basis_set)
    expected_integer_grid_points = [[0], [1], [2], [3], [4]]
    assert (test_class.make_integer_grid_points(1)
            == expected_integer_grid_points)


def test_make_integer_grid_points():
    ...
    assert (test_class.make_integer_grid_points(2)
            == expected_integer_grid_points)
...
def test_smolyak_indicies_one_dimension():
    ...
    assert test_class.smolyak_indices(1) == expected_smolyak_indices
...
def test_smolyak_indices():
    ...
    assert test_class.smolyak_indices(2) == expected_smolyak_indices
...
def test_smolyak_indices_expand():
    ...
    assert (test_class.smolyak_indices(2, expand_indicy=[1, 4],
...
def test_smolyak_indices_drop():
    ...
    assert (test_class.smolyak_indices(2, expand_indicy=[],
...
def test_smolyak_indices_dimension_error():
    ...
    with pytest.raises(IndexError):
        test_class.smolyak_indices(2, expand_indicy=[1, 1, 1],
                                   drop_indicy=[1, 2, 3])
...
def test_smolyak_indices_drop_error():
    ...
    with pytest.raises(ValueError):
        test_class.smolyak_indices(2, drop_indicy=[1, 5])
