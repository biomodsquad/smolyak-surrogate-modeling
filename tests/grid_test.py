...
def test_adaptive_smolyak_grid_1d():
    ...
    grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(3),[[2]])
    grid = grid_gen(1)
    assert grid_gen.smolyak_indexes == [[2]]
    assert grid.indexes == [1,2]
    assert grid.levels == [1, 1]
    assert numpy.allclose(grid.points, [-1, 1])

def test_adaptive_smolyak_grid_2d():
    """Test grids generated by adaptive Smolyak method."""
    grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(2),[[1,1],[1,2]])
    grid = grid_gen(2)
    assert grid_gen.smolyak_indexes == [[1,1],[1,2]]
    assert grid.indexes == [[0, 0],[0, 1],[0, 2]]
    assert numpy.allclose(grid.points,[[0, 0],[0, -1],[0, 1]])

def test_adaptive_smolyak_grid_initialize_indexerror():
    """Test if grid errors if given indices it doesn't have points for."""
    with pytest.raises(IndexError):
        grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(3),[[1, 1],[1, 2],[1, 3],[1, 4],[1, 5]])

def test_adaptive_smolyak_grid_initialize_valueerror():
    """Test if grid errors if given indices with inconsistent dimensions."""
    with pytest.raises(ValueError):
        grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(3),[[1, 1],[1, 2],[1, 3],[1, 4],[1, 1, 2]])

def test_adaptive_smolyak_grid_smolyak_indexes_setter():
    """Test grids generated by adaptive Smolyak method."""
    grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(2),[[1,1],[1,2]])
    grid_gen.smolyak_indexes = [[1,1],[1,2],[2,1]]
    grid = grid_gen(2)
    assert grid_gen.smolyak_indexes == [[1,1],[1,2],[2,1]]
    assert grid.indexes == [[0, 0],[0, 1],[0, 2],[1,0],[2,0]]
    assert numpy.allclose(grid.points,[[0,0],[0,-1],[0,1],[-1,0],[1,0]])

def test_adaptive_smolyak_grid_smolyak_indexes_setter_error():
    """Test grids generated by adaptive Smolyak method."""
    grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(2),[[1,1],[1,2]])
    with pytest.raises(IndexError):
        grid_gen.smolyak_indexes = [[1,1],[1,2],[4,1]]
    with pytest.raises(ValueError):
        grid_gen.smolyak_indexes = [[1,1,1],[1,2]]

def test_adaptive_smolyak_grid_dim_error():
    """Test grids generated by adaptive Smolyak method."""
    grid_gen = AdaptiveSmolyakGridGenerator(ChebyshevFirstKind.make_nested_set(2),[[1,1],[1,2]])
    with pytest.raises(ValueError):
        grid = grid_gen(3)
